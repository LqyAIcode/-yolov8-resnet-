...
if __name__ == '__main__':
    ...
    # 保存分类模型和回归模型的权重
    torch.save(classification_model.state_dict(), 'classification_model_2.pth')
    torch.save(regression_model.state_dict(), 'regression_model_2.pth')
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            images, regression_labels, classification_labels = images.to(device), labels["regression_label_2"].to(device), \
                labels["classification_label_2"].to(device)
            ...
            #     1000
            #     )
            # print("..................",min_values[inconsistent_mask])
            # final_regression_preds[inconsistent_mask] = torch.where(
